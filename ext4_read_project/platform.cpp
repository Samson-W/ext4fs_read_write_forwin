...
//hnd:设备句柄  ptr:读出的内容存储的空间  sector:要读取的起始扇区   nsects:要读取的扇区数目  sectorsize:一个扇区的大小
...
int write_disk(FileHandle hnd, void *ptr, lloff_t sector, int nsects, int sectorsize)
{
    ...
    low = SetFilePointer(hnd, low, &high, FILE_BEGIN/*FILE_CURRENT*/);

    len = nsects * sectorsize;
    ret = WriteFile(hnd, ptr, len, &rd, NULL);
    if(!ret)
        return -1;
